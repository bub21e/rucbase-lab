...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    std::scoped_lock lock{latch_};
    bool exist = page_table_.find(page_id) != page_table_.end();
    
    if (exist) {
        
        frame_id_t frame_id_ = page_table_[page_id];
        Page* page = &pages_[frame_id_];
...
void BufferPoolManager::flush_all_pages(int fd) {
    ...
    for (auto it = page_table_.begin() ; it != page_table_.end() ; it++) {
        ...
        if(page_id.fd == fd){
            disk_manager_->write_page(fd , page_id.page_no , pages_[frame_id].get_data() , PAGE_SIZE);
        }
    }
}